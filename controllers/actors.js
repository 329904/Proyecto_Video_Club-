...
function list(req, res, next){
    Actor.findAll({include: ['movies']}).then(objects => res.json(objects)).catch(err => res.send(err));
}

function index(req, res, next){
    const id = req.params.id;
    Actor.findByPk(id).then(object => res.json(object)).catch(err => res.send(error));
}
...
function replace(req, res, next){
    ...
    Actor.findByPk(id).then((object)=>{
        ...
        object.update({name:name, lastName:lastName}).then(actor => res.json(actor)).catch(err => res.send(err));
